...
class BtPosition:
    ...
    take_profit: float
    entry_bar: int
    entry_ts: object = None                        # pd.Timestamp，供月度統計用
    tp1_hit: bool = False
    bars_held: int = 0
...
class BtTrade:
    ...
    entry_ts: object = None                        # pd.Timestamp
...
class Backtester:
    ...
    FEE_RATE = 0.0007  # 0.05% taker + 0.02% slippage per side（實盤對齊）
    ...
    def fetch_data(symbol: str, days: int = 365) -> pd.DataFrame:
        """從 Binance 公開接口抓取 1h OHLCV（分頁，無上限）"""
        ...
        while len(all_bars) < needed:
            chunk = ex.fetch_ohlcv(symbol, "1h", since=since_ms, limit=1000)
            if not chunk:
                break
            all_bars += chunk
            if len(chunk) < 1000:
                break
            since_ms = chunk[-1][0] + 3_600_000  # 下一根開始時間
        # 去重並裁切
        seen = set()
        deduped = []
        for bar in all_bars:
            if bar[0] not in seen:
                seen.add(bar[0])
                deduped.append(bar)
        deduped = deduped[:needed]
        df = pd.DataFrame(
            deduped, columns=["timestamp", "open", "high", "low", "close", "volume"]
        )
    ...
    def _open(self, setup: Dict, bar_idx: int, indicators: Dict, ts=None):
        qty  = setup["quantity"]
    ...
    def _partial_close(self, exit_price: float, pos: BtPosition):
        """TP1 分批平倉 50%，止損移至成本+0.2ATR（防止插針洗出）"""
        ...
        atr = pos.indicators.get("atr", pos.entry_price * 0.01)
        be_buffer = 0.2 * atr
        if pos.side == "long":
            pos.stop_loss = pos.entry_price + be_buffer
        else:
            pos.stop_loss = pos.entry_price - be_buffer
        ...
        self.trades.append(BtTrade(
            ...
            trade_id=pos.entry_bar, entry_ts=pos.entry_ts,
    ...
    def report(self) -> str:
        ...
        for tid, grp in groups.items():
            ...
            if key not in month_data:
                month_data[key] = {"pnl": 0.0, "wins": 0, "total": 0, "full_wins": 0}
            reasons_grp = {t.exit_reason for t in grp}
            group_pnl   = sum(t.pnl for t in grp)
            month_data[key]["pnl"]   += group_pnl
            month_data[key]["total"] += 1
            if group_pnl > 0:
                month_data[key]["wins"] += 1
            if "tp" in reasons_grp:
                month_data[key]["full_wins"] += 1

        if month_data:
            lines.append("-" * 58)
            lines.append("  月度損益分佈:")
            lines.append(f"  {'月份':<8} {'損益(USDT)':>12} {'勝率':>7} {'次數':>5} {'完整獲利':>6}")
            cumulative = self.initial_balance
            for mkey in sorted(month_data.keys()):
                md   = month_data[mkey]
                mpnl = md["pnl"]
                ...
                cumulative += mpnl
                lines.append(
                    f"  {mkey:<8} {sign}{mpnl:>11.2f} {wr:>6.0f}% {md['total']:>5} {md['full_wins']:>7}次"
                )
            lines.append(f"  {'累計':8} {cumulative - self.initial_balance:>+12.2f}")
